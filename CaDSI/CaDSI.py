...
class CaDSI(object):
    def __init__(self, data_config, pretrain_data):
        ...
        #add aspect embeddings and its num 

        self.A1_vec_aspect_id_num=config['n_A1_aspect'] 
        self.A1_vec_uid_num=config['n_A1_aspect_u']

        self.A1_vec_aspect_id=config['A1_aspect_list']
        self.A1_vec_uid=config['A1_aspect_u_list']

        self.A2_vec_aspect_id_num=config['n_A2_aspect'] 
        self.A2_vec_uid_num=config['n_A2_aspect_u']

        self.A2_vec_aspect_id=config['A2_aspect_list']
        self.A2_vec_uid=config['A2_aspect_u_list']

        self.A3_vec_aspect_id_num=config['n_A3_aspect'] 
        self.A3_vec_uid_num=config['n_A3_aspect_u']

        self.A3_vec_aspect_id=config['A3_aspect_list']
        self.A3_vec_uid=config['A3_aspect_u_list']

        self.A4_vec_aspect_id_num=config['n_A4_aspect'] 
        self.A4_vec_uid_num=config['n_A4_aspect_u']

        self.A4_vec_aspect_id=config['A4_aspect_list']
        self.A4_vec_uid=config['A4_aspect_u_list']


        self.A1_vec=config['A1_vec']
        self.A2_vec=config['A2_vec']
        self.A3_vec=config['A3_vec']
        self.A4_vec=config['A4_vec']

        self.A1_vec_user=config['A1_vec_user']
        self.A1_vec_aspect=config['A1_vec_aspect']

        self.A2_vec_user=config['A2_vec_user']
        self.A2_vec_aspect=config['A2_vec_aspect']

        self.A3_vec_user=config['A3_vec_user']
        self.A3_vec_aspect=config['A3_vec_aspect']

        self.A4_vec_user=config['A4_vec_user']
        self.A4_vec_aspect=config['A4_vec_aspect']
        ...
        *********************************************************
        ...
        # create models
        self.ua_embeddings, self.ia_embeddings, self.f_weight, self.ua_embeddings_t, self.ia_embeddings_t = self._disentangle_intent_learning(pick_=self.is_pick)
        ...
        self.A1_vec_user = tfv1.convert_to_tensor(self.A1_vec_user)
        self.A1_vec_aspect= tfv1.convert_to_tensor(self.A1_vec_aspect)

        self.A2_vec_user = tfv1.convert_to_tensor(self.A2_vec_user)
        self.A2_vec_aspect= tfv1.convert_to_tensor(self.A2_vec_aspect)

        self.A3_vec_user = tfv1.convert_to_tensor(self.A3_vec_user)
        self.A3_vec_aspect= tfv1.convert_to_tensor(self.A3_vec_aspect)

        self.A4_vec_user = tfv1.convert_to_tensor(self.A4_vec_user)
        self.A4_vec_aspect= tfv1.convert_to_tensor(self.A4_vec_aspect)

        self.A1_user_embeddings = tf.nn.embedding_lookup(self.A1_vec_user, self.users)
        self.A2_user_embeddings = tf.nn.embedding_lookup(self.A2_vec_user, self.users)
        self.A3_user_embeddings = tf.nn.embedding_lookup(self.A3_vec_user, self.users)
        self.A4_user_embeddings = tf.nn.embedding_lookup(self.A4_vec_user, self.users)

        self.batch_ratings = tf.matmul(self.u_g_embeddings_t, self.pos_i_g_embeddings_t, transpose_a=False, transpose_b=True)
        self.batch_ratings_A = tf.matmul (self.A1_user_embeddings,self.pos_i_g_embeddings_t, transpose_a=False, transpose_b=True) + self.batch_ratings
        
        self.mf_loss, self.emb_loss = self.create_loss_function_bpr(self.u_g_embeddings,self.A1_user_embeddings,self.A2_user_embeddings,self.A3_user_embeddings,self.A4_user_embeddings, self.pos_i_g_embeddings, self.neg_i_g_embeddings)

        # whether user distance correlation
        if args.corDecay < 1e-9:
            self.cor_loss = tf.constant(0.0)
        else:
            self.cor_loss = args.corDecay * self.create_loss_function_cor(self.cor_u_g_embeddings, self.cor_i_g_embeddings)                   
    ...
    def numpy_xavier_initialise(n, dim):
        np.random.seed(0)
        scale = 1/max(1., (2+2)/2.)
    ...
    def _init_weights(self):
        ...
        initializer = tfv1.glorot_uniform_initializer()
    ...
    def _disentangle_intent_learning(self, pick_ = False):
        ...
        print("Dimension user embeddings")
        print(self.weights['user_embedding'])

        print("Dimension item embeddings")
        print(self.weights['item_embedding'].shape)
    ...
    def create_loss_function_bpr(self, users, aspect_users_A1,aspect_users_A2,aspect_users_A3,aspect_users_A4, pos_items, neg_items):
        aspect_pos_scores_A1=tf.multiply(aspect_users_A1,pos_items)
        aspect_neg_scores_A1=tf.multiply(aspect_users_A1,neg_items)
        aspect_pos_scores_A2=tf.multiply(aspect_users_A2,pos_items)
        aspect_neg_scores_A2=tf.multiply(aspect_users_A2,neg_items)
        aspect_pos_scores_A3=tf.multiply(aspect_users_A3,pos_items)
        aspect_neg_scores_A3=tf.multiply(aspect_users_A3,neg_items)
        aspect_pos_scores_A4=tf.multiply(aspect_users_A4,pos_items)
        aspect_neg_scores_A4=tf.multiply(aspect_users_A4,neg_items)
        pos_scores = tf.reduce_sum(tf.multiply(users, pos_items) + aspect_pos_scores_A1 + aspect_pos_scores_A2 + aspect_pos_scores_A3 + aspect_pos_scores_A4, axis=1)
        neg_scores = tf.reduce_sum(tf.multiply(users, neg_items) + aspect_neg_scores_A1 + aspect_neg_scores_A2 + aspect_neg_scores_A3 + aspect_pos_scores_A4, axis=1)
...
if __name__ == '__main__':
    ...
    config['n_A1_aspect'] = data_generator.A1_vec_aspect_id_num
    config['n_A1_aspect_u'] =data_generator.A1_vec_uid_num

    config['A1_aspect_list'] = data_generator.A1_vec_aspect_id
    config['A1_aspect_u_list'] =data_generator.A1_vec_uid

    config['n_A2_aspect'] = data_generator.A2_vec_aspect_id_num
    config['n_A2_aspect_u'] =data_generator.A2_vec_uid_num

    config['A2_aspect_list'] = data_generator.A2_vec_aspect_id
    config['A2_aspect_u_list'] =data_generator.A2_vec_uid

    config['n_A3_aspect'] = data_generator.A3_vec_aspect_id_num
    config['n_A3_aspect_u'] =data_generator.A3_vec_uid_num

    config['A3_aspect_list'] = data_generator.A3_vec_aspect_id
    config['A3_aspect_u_list'] =data_generator.A3_vec_uid

    config['n_A4_aspect'] = data_generator.A4_vec_aspect_id_num
    config['n_A4_aspect_u'] =data_generator.A4_vec_uid_num

    config['A4_aspect_list'] = data_generator.A4_vec_aspect_id
    config['A4_aspect_u_list'] =data_generator.A4_vec_uid

    config['A1_vec']=data_generator.A1_vec
    config['A2_vec']=data_generator.A2_vec
    config['A3_vec']=data_generator.A3_vec
    config['A4_vec']=data_generator.A4_vec

    config['A1_vec_user']=data_generator.A1_vec_user
    config['A1_vec_aspect']=data_generator.A1_vec_aspect

    config['A2_vec_user']=data_generator.A2_vec_user
    config['A2_vec_aspect']=data_generator.A2_vec_aspect

    config['A3_vec_user']=data_generator.A3_vec_user
    config['A3_vec_aspect']=data_generator.A3_vec_aspect

    config['A4_vec_user']=data_generator.A4_vec_user
    config['A4_vec_aspect']=data_generator.A4_vec_aspect


    print("************************* loading meta_path vec aspect with following setting ***************************")
    print('num of aspects_A1:',config['n_A1_aspect'], 'num of users:',config['n_A1_aspect_u'],'\n')
    print('dim of aspect_A1: ','aspect: ',len(config['A1_vec_aspect']), 'user: ',len(config['A1_vec_user']),'\n')
    print('num of aspects_A2:',config['n_A2_aspect'], 'num of users:',config['n_A2_aspect_u'],'\n')
    print('dim of aspect_A2: ','aspect: ',len(config['A2_vec_aspect']), 'user: ',len(config['A2_vec_user']),'\n')
    print('num of aspects_A3:',config['n_A3_aspect'], 'num of users:',config['n_A3_aspect_u'],'\n')
    print('dim of aspect_A3: ','aspect: ',len(config['A3_vec_aspect']), 'user: ',len(config['A3_vec_user']),'\n')
    print('dim of aspect_A4: ','aspect: ',len(config['A4_vec_aspect']), 'user: ',len(config['A4_vec_user']),'\n')
    print("*************************------------------------------------------------------***************************")
    ...
    *********************************************************
    ...
    model = CaDSI(data_config=config, pretrain_data=pretrain_data)
    ...
    *********************************************************
    ...
    for epoch in range(args.epoch):
        ...
        n_batch = data_generator.n_train // args.batch_size + 1 #self.n_train += len(items)
        ...
        for idx in range(n_batch):
            ...
            print('loss:',loss, 'mf_loss:',mf_loss,'emb_loss:',emb_loss, 'cor_loss:',cor_loss )
